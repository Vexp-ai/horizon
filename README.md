...
- Toolchain caveat: recent `llama-server` builds can silently skip
  speculative decoding (unified KV cache default) while still loading the
  draft model into RAM. Verify that acceptance stats are non-zero; the
  `llama-speculative` binary is the reliable measurement path today
